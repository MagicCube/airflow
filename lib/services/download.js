...
module.exports = {
  async createTask(task) {
    const gid = await aria2.addUri(task.uris, task.options);
    const newTask = await aria2.tellStatus(gid, KEYS);
    addTask(newTask);
    log.info(`New task[${gid}, ${task.uris}, ${task.options.dir}] has been added to task list.`);
    return newTask;
  },
